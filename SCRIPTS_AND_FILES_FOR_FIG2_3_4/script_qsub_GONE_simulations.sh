...
#Scratch directory
mkdir -p /state/partition1/GONE_SIMS$d/$SLURM_JOBID/
mkdir -p /state/partition1/GONE_SIMS$d/$SLURM_JOBID/PROGRAMMES
...
for ((n=1; n<=$NCHR; n++))
do
cp chromosome$n.* /state/partition1/GONE_SIMS$d/$SLURM_JOBID/
done

cp ../INPUT_PARAMETERS_FILE_SIM /state/partition1/GONE_SIMS$d/$SLURM_JOBID/
cp ../PROGRAMMES/LD_SNP_REAL3 /state/partition1/GONE_SIMS$d/$SLURM_JOBID/PROGRAMMES/
cp ../PROGRAMMES/SUMM_REP_CHROM3 /state/partition1/GONE_SIMS$d/$SLURM_JOBID/PROGRAMMES/
cp ../PROGRAMMES/GONE /state/partition1/GONE_SIMS$d/$SLURM_JOBID/PROGRAMMES/
cp ../PROGRAMMES/GONEaverage /state/partition1/GONE_SIMS$d/$SLURM_JOBID/PROGRAMMES/
cp ../PROGRAMMES/GONEparallel.sh /state/partition1/GONE_SIMS$d/$SLURM_JOBID/PROGRAMMES/

touch $WDIR/$SLURM_JOBID.`hostname`.`date +%HH%MM`
cd /state/partition1/GONE_SIMS$d/$SLURM_JOBID
...
for ((n=1; n<=$NCHR; n++))
do
cp chromosome$n.* $WDIR/OUTPUT_GONE_$FILE/
done
...
./PROGRAMMES/GONEparallel.sh -hc $hc -lc 0.0005 outfileLD $REPS
...
cp -r /state/partition1/GONE_SIMS$d/$SLURM_JOBID/outfileLD_Ne_estimates $WDIR/Ne_$FILE
cp -r /state/partition1/GONE_SIMS$d/$SLURM_JOBID/outfileLD_d2_sample $WDIR/d2_$FILE
cp -r /state/partition1/GONE_SIMS$d/$SLURM_JOBID/outfileLD_TEMP $WDIR/OUTPUT_GONE_$FILE/
cp -r /state/partition1/GONE_SIMS$d/$SLURM_JOBID/PARAMETERS_$FILE $WDIR/

######################## SCRATCH CLEANING #########################

rm $WDIR/$SLURM_JOBID.* 2> /dev/null
rm -rf /state/partition1/GONE_SIMS$d/$SLURM_JOBID/ 2> /dev/null
